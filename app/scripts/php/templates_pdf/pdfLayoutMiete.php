...
?>
		...
		<div class="content">
			...
			<div class="frontOverview">
				...
				<?php 
					...
					if(isset($content["pauschalmiete"]) && $content["pauschalmiete"]>0){ ?>
					<tr>
						<td>Pauschalmiete</td><td><?php echo number_format($content["pauschalmiete"],2, ',', '.'); ?> EUR</td>
					</tr>
				<?php }else{ ?>
					<tr>
						<td>Nettokaltmiete</td><td><?php echo number_format($content["kaltmiete"],2, ',', '.'); ?> EUR</td>
					</tr><tr>	
						<td>Monatsnebenkosten</td><td><?php echo number_format($content["nebenkosten"],2, ',', '.'); ?> EUR</td>
					</tr>
				<?php  } ?>
		...
		<div class="content">
			...
			<div class="container" style="height:180px;">
				...
				<div class="maindata">
					...
					<div class="tableWrapper">
						<table>
						<?php if(isset($content["nebenkosten"]) && $content["nebenkosten"]>0){ ?>
							<tr>
								<td>&bull;</td><td>Nebenkosten</td><td>ca. <?php echo number_format($content["nebenkosten"],2, ',', '.'); ?> EUR</td>
							</tr>
						<? } ?>
							<tr>
			...
			<div class="container">
				<div class="halfBox">
					...
					<p style="margin-right:5px;font-size:10px;word-break: normal;text-align:justify;display: none;">
						...
						auf der Basis des Objekt-Exposés und seiner Bedingungen zustande. Die Courtage in Höhe der 2,38 vereinbarten Nettokaltmiete 
						(bei Staffelmiete bezogen auf die Durchschnittsnettokalt-miete der Vertragslaufzeit) inkl. gesetzlicher Mehrwertsteuer (z. Zt. 19%) ist mit 
						Abschluss des Mietvertrages verdient und fällig und vom Mieter alleine zu tragen. Die EuV Residential Berlin GmbH und ggf. deren Beauftragte 
						erhalten einen unmittelba-ren Zahlungsanspruch gegenüber dem Mieter (Vertrag zugunsten Dritter, § 328 BGB).
					</p>
				</div>
				<div class="halfBox">
	...
	<div class="exposePage">	
	<!--  zweite Seite - Ausstattung und Lage -->
		<div class="header">
			<table class="headerTable">
				<tr>
					<td style="padding-bottom: -2px;">
		...
		<div class="content">
			...
			<div class="container">
			<?php 
				if(isset($content["manualTextObjekt"]) && !empty($content["manualTextObjekt"])){
			?>
				<span class="subTitle">Objektbeschreibung</span>
				<p class="block">
					<?php echo $content["manualTextObjekt"]; ?>
				</p>
			<?php } ?>
			</div>
			<div class="container">
					...
					<div class="tableWrapper2">
						<table>
							...
							<?php 
								...
								$ausstattungboden = array_merge($arrAusstattung,$innenausstattung, $boden);								
								$i=1;
			...
			<div class="container">
			<?php 
				if(isset($content["manualTextLage"]) && !empty($content["manualTextLage"])){
			?>
				<span class="subTitle">Lage</span>
				<p class="block">
					<?php echo $content["manualTextLage"];?>
				</p>
			<?php } ?>
			</div>
			...
			<div class="maindata">			
				...
				if($gall!==false && is_array($gall)){		
					...
					foreach($gall["images"] as $x=>$imgObj){
						...
						echo '<td style="width:'.$tdWidth.'%;">';
						?>									
							<div class="imageWrapper" style="height:360px">
								<div>
								<?php
									echo $imgObj->showImage($ic->__get("imageWrapper"));
									echo $imgObj->showTitle($ic->__get("imageWrapper"));
